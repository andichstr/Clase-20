...
export class ProductManager {
    ...
    async addProduct(product) {
        ...
        if (this.isValidProduct(product)) {
            ...
            if (!this.isCodeRepeated(products, product.code)){
                ...
                logger.error(`Error al agregar producto ingresado con codigo ${product.code}: Codigo repetido.`)
                return {error: -1};
            }
    ...
    async getProducts() {
        ...
        try {
            ...
            logger.error(`Error al leer el archivo de productos: ${error.message}`);
            return [];
        }
    }
    ...
    async updateProduct(id, product) {
        ...
        if (this.hasValidProp(product)) {
            if (this.isCodeRepeated(products, product.code)){
                logger.info(`El codigo del producto ${JSON.stringify(product)} esta repetido`);
                return -3;
            }
            let found = false;
            let i = 0;
            ...
            logger.warning(`El producto ${JSON.stringify(product)} no contiene ninguna propiedad valida para actualizar`);
            return -1;
        };
    }
